...
beforeAll(async () => {
  ...
  const spot = await createSpot({ name: `Update Spot Test ${suffix}`, countryCode: "FR" });
  spotId = spot.id;
});
...
describe("updateSpot (integration)", () => {
  ...
  it("clears an optional field when it is omitted from the input", async () => {
    const updated = await updateSpot(spotId, { name: "Updated Spot Name" });
  ...
  it("fails with invalid data", async () => {
    await expect(updateSpot(spotId, { name: "" })).rejects.toThrow();
  });

  it("fails when the spot does not exist", async () => {
    await expect(updateSpot(crypto.randomUUID(), { name: "Anything" })).rejects.toThrow();
  });
});
