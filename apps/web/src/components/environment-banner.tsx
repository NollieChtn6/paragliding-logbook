...
function getEnvironmentLabel(t: ReturnType<typeof getDictionary>["environment"]): string | null {
  const vercelEnv = process.env.VERCEL_ENV;
  if (vercelEnv === "production") return null;
  if (vercelEnv === "preview") return t.preview;
  return t.local;
}
...
// test avec la production pendant une campagne de tests manuels. locale reçu
// en prop (déjà résolu par app/layout.tsx) plutôt que lu ici : évite une
// deuxième lecture du cookie pour ce composant très en amont dans l'arbre.
export function EnvironmentBanner({ locale }: EnvironmentBannerProps) {
  const t = getDictionary(locale).environment;
  const label = getEnvironmentLabel(t);
